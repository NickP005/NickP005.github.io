# PLYNX - INFORMATIVA SULLA PRIVACY

**Ultimo aggiornamento: 26 novembre 2025**

Questa Informativa sulla Privacy ("Informativa") descrive come Plynx ("Plynx", "noi", "ci" o "nostro") raccoglie, utilizza, divulga e protegge le tue informazioni quando utilizzi l'applicazione mobile Plynx ("App") e i servizi correlati (collettivamente, i "Servizi").

Siamo impegnati a proteggere la tua privacy e a gestire i tuoi dati con trasparenza. Ti preghiamo di leggere attentamente questa Informativa.

---

## SOMMARIO

In breve:
- Raccogliamo dati personali minimi: indirizzo email e hash della password per l'autenticazione
- Raccogliamo dati tecnici: indirizzi IP, token dei dispositivi, configurazioni di widget/dashboard
- NON raccogliamo il tuo nome, numero di telefono, indirizzo fisico o informazioni di pagamento (a meno che tu non li fornisca)
- NON vendiamo i tuoi dati a terzi
- NON utilizziamo analytics di terze parti nell'App (solo statistiche standard dell'App Store)
- NON condividiamo i tuoi dati per scopi di marketing
- Puoi richiedere la cancellazione dei tuoi dati in qualsiasi momento

---

## INDICE

1. Chi Siamo
2. Quali Informazioni Raccogliamo
3. Come Utilizziamo le Tue Informazioni
4. Come Archiviamo le Tue Informazioni
5. Come Condividiamo le Tue Informazioni
6. Conservazione dei Dati
7. I Tuoi Diritti e Scelte
8. Sicurezza dei Dati
9. Trasferimenti Internazionali di Dati
10. Privacy dei Minori
11. Link e Servizi di Terze Parti
12. Diritti Privacy California (CCPA)
13. Diritti Privacy Europei (GDPR)
14. Modifiche a Questa Informativa
15. Contattaci

---

## 1. CHI SIAMO

Plynx è sviluppato e gestito da:

**Niccolò Pagano (NickP005)**  
Email: niccolo.pagano@studbocconi.it  
Sito web: https://plynx.cc

Ai fini del GDPR, Niccolò Pagano è il **Titolare del Trattamento** per i dati personali raccolti attraverso il Plynx Cloud Server (plynx.cc).

Se utilizzi un server self-hosted, l'operatore di quel server è il Titolare del Trattamento.

---

## 2. QUALI INFORMAZIONI RACCOGLIAMO

### 2.1 Informazioni che Fornisci Tu

Quando crei un account o utilizzi i Servizi, raccogliamo:

| Tipo di Dato | Scopo | Obbligatorio |
|--------------|-------|--------------|
| **Indirizzo Email** | Identificazione account, login, recupero password, comunicazioni di servizio | Sì |
| **Password** | Autenticazione (memorizzata come hash crittografico, non in chiaro) | Sì |

**NON richiediamo né raccogliamo:**
- Nome completo
- Numero di telefono
- Indirizzo fisico
- Data di nascita
- Informazioni di pagamento (l'app è gratuita)
- Numeri di documenti di identità

### 2.2 Informazioni Raccolte Automaticamente

Quando utilizzi il Plynx Cloud Server, raccogliamo automaticamente:

| Tipo di Dato | Scopo | Memorizzato |
|--------------|-------|-------------|
| **Indirizzo IP** | Sicurezza, prevenzione frodi, rilevamento abusi | Sì (IP di registrazione e ultimo login) |
| **Timestamp Ultimo Login** | Sicurezza account, monitoraggio attività | Sì |
| **Token Dispositivi** | Autenticazione hardware | Sì |
| **Identificatore App** | Distinguere tra diverse applicazioni client | Sì |
| **Regione** | Ottimizzazione server (se applicabile) | Sì |

### 2.3 Dati dei Dispositivi IoT

Quando i tuoi dispositivi hardware si connettono alla piattaforma Plynx, possiamo raccogliere e memorizzare:

| Tipo di Dato | Scopo | Memorizzato |
|--------------|-------|-------------|
| **Token di Autenticazione** | Autenticazione dispositivo | Sì |
| **Dati Widget** | Visualizzare valori nelle dashboard (letture sensori, stati, ecc.) | Sì |
| **Dati Storici** | Dati serie temporali per grafici (se usi widget SuperChart) | Sì |
| **Stato Connessione Dispositivo** | Mostrare stato online/offline | Temporaneamente |
| **Ultimo IP Dispositivo** | Sicurezza e debugging | Sì |

### 2.4 Informazioni Memorizzate Localmente sul Tuo Dispositivo

L'App Plynx memorizza alcune informazioni localmente sul tuo dispositivo utilizzando storage sicuro (Keychain su iOS):

- Indirizzo email
- Password (crittografata)
- Configurazione server (host, porta)
- Token di accesso
- Preferenze dashboard

Questi dati sono memorizzati localmente e non vengono trasmessi a noi se non per autenticarsi e utilizzare i Servizi.

### 2.5 Informazioni che NON Raccogliamo

Vogliamo essere chiari su cosa NON raccogliamo:

- Dati di posizione (GPS)
- Contatti
- Foto o file multimediali
- Accesso a microfono o fotocamera
- Identificatori pubblicitari
- Analytics di terze parti (no Firebase Analytics, Google Analytics, ecc.)
- Cronologia di navigazione
- Profili social media

---

## 3. COME UTILIZZIAMO LE TUE INFORMAZIONI

Utilizziamo le informazioni raccolte per i seguenti scopi:

### 3.1 Per Fornire i Servizi
- Creare e gestire il tuo account
- Autenticare il tuo login
- Connettere i tuoi dispositivi hardware
- Visualizzare le tue dashboard e dati widget
- Memorizzare dati storici per i grafici

### 3.2 Per Mantenere e Migliorare i Servizi
- Monitorare le prestazioni del servizio
- Risolvere problemi tecnici
- Migliorare l'esperienza utente

### 3.3 Per Comunicare con Te
- Inviare email di recupero password
- Notificarti aggiornamenti importanti del servizio
- Rispondere alle tue richieste di supporto

### 3.4 Per Garantire la Sicurezza
- Rilevare e prevenire frodi
- Identificare accessi non autorizzati
- Proteggere da abusi

### 3.5 Per Adempiere agli Obblighi Legali
- Rispondere a richieste legittime delle autorità
- Conformarsi alle leggi e regolamenti applicabili

**NON utilizziamo le tue informazioni per:**
- Pubblicità o marketing
- Vendita a terzi
- Profilazione per scopi commerciali
- Processi decisionali automatizzati che influenzano i tuoi diritti

---

## 4. COME ARCHIVIAMO LE TUE INFORMAZIONI

### 4.1 Posizione di Archiviazione

Se utilizzi il **Plynx Cloud Server** (plynx.cc):
- I tuoi dati sono memorizzati su server situati in Europa
- Utilizziamo misure di sicurezza standard del settore

Se utilizzi un **server self-hosted**:
- I tuoi dati sono memorizzati sulla tua infrastruttura
- Sei responsabile della sicurezza dei dati e della conformità

### 4.2 Misure di Sicurezza dei Dati

Implementiamo misure di sicurezza ragionevoli, incluse:
- Hashing delle password (non memorizzate in chiaro)
- Connessioni crittografate (TLS/SSL)
- Controlli di accesso
- Revisioni di sicurezza regolari

---

## 5. COME CONDIVIDIAMO LE TUE INFORMAZIONI

### 5.1 NON Vendiamo i Tuoi Dati

**NON vendiamo, affittiamo o scambiamo le tue informazioni personali a terzi per marketing o qualsiasi altro scopo.**

### 5.2 Condivisione Limitata

Possiamo condividere le tue informazioni solo in queste circostanze limitate:

| Destinatario | Scopo | Tipo di Dati |
|--------------|-------|--------------|
| **Fornitori di Servizi** | Hosting infrastruttura | Dati tecnici |
| **Autorità Legali** | Quando richiesto dalla legge o procedimento legale | Come legalmente richiesto |
| **Successori** | In caso di fusione, acquisizione o vendita di asset | Tutti i dati (con notifica) |

### 5.3 Dati Aggregati

Possiamo condividere dati aggregati e anonimizzati che non possono identificarti (es. numero totale di utenti, statistiche generali di utilizzo).

---

## 6. CONSERVAZIONE DEI DATI

### 6.1 Account Attivi

Conserviamo i tuoi dati finché il tuo account è attivo o necessario per fornire i Servizi.

### 6.2 Cancellazione Account

Quando cancelli il tuo account:
- I tuoi dati personali (email, hash password) vengono cancellati
- Le tue dashboard e configurazioni widget vengono cancellate
- I dati storici vengono cancellati
- Alcuni log anonimizzati possono essere conservati per scopi di sicurezza

### 6.3 Periodi di Conservazione

| Tipo di Dato | Periodo di Conservazione |
|--------------|-------------------------|
| Dati account | Fino alla cancellazione dell'account |
| Dati storici/grafici | Fino alla cancellazione dell'account |
| Log server | Fino a 90 giorni |
| Dati di backup | Fino a 30 giorni dopo la cancellazione |

---

## 7. I TUOI DIRITTI E SCELTE

Hai i seguenti diritti riguardo ai tuoi dati personali:

### 7.1 Accesso
Puoi richiedere una copia dei tuoi dati personali.

### 7.2 Rettifica
Puoi aggiornare o correggere dati inaccurati.

### 7.3 Cancellazione
Puoi richiedere la cancellazione del tuo account e dati.

### 7.4 Portabilità dei Dati
Puoi richiedere i tuoi dati in un formato portatile (esportazione JSON).

### 7.5 Limitazione
Puoi richiedere di limitare come utilizziamo i tuoi dati.

### 7.6 Opposizione
Puoi opporti a determinati trattamenti dei tuoi dati.

### 7.7 Revoca del Consenso
Dove il trattamento è basato sul consenso, puoi revocarlo in qualsiasi momento.

**Per esercitare uno qualsiasi di questi diritti, contattaci a: niccolo.pagano@studbocconi.it**

Risponderemo alla tua richiesta entro 30 giorni (o come richiesto dalla legge applicabile).

---

## 8. SICUREZZA DEI DATI

Implementiamo misure di sicurezza amministrative, tecniche e fisiche ragionevoli per proteggere le tue informazioni, incluse:

- Hashing crittografico delle password
- Trasmissione dati crittografata (TLS)
- Meccanismi di autenticazione sicuri
- Valutazioni di sicurezza regolari

Tuttavia, nessun sistema è completamente sicuro. Non possiamo garantire la sicurezza assoluta dei tuoi dati. Sei responsabile di:
- Mantenere riservata la tua password
- Proteggere i tuoi dispositivi e token
- Segnalare eventuali incidenti di sicurezza

---

## 9. TRASFERIMENTI INTERNAZIONALI DI DATI

Se accedi ai Servizi dall'esterno del paese dove si trovano i nostri server, i tuoi dati potrebbero essere trasferiti a livello internazionale.

### 9.1 Per gli Utenti UE
Il Plynx Cloud Server (plynx.cc) è situato in Europa. I tuoi dati rimangono all'interno dello Spazio Economico Europeo (SEE).

Se dovessimo trasferire dati al di fuori del SEE, garantiamo che siano in atto garanzie appropriate, come:
- Clausole Contrattuali Standard (SCC)
- Decisioni di adeguatezza della Commissione Europea
- Il tuo consenso esplicito

### 9.2 Per gli Utenti Extra-UE
Se accedi ai Servizi dall'esterno dell'Unione Europea, i tuoi dati vengono trasferiti e memorizzati sui nostri server in Europa.

---

## 10. PRIVACY DEI MINORI

I Servizi non sono destinati a minori di 16 anni (o l'età applicabile del consenso digitale nella tua giurisdizione).

Non raccogliamo consapevolmente informazioni personali da minori. Se scopriamo di aver raccolto dati da un minore, li cancelleremo tempestivamente.

Se ritieni che abbiamo inavvertitamente raccolto dati da un minore, contattaci immediatamente a niccolo.pagano@studbocconi.it.

---

## 11. LINK E SERVIZI DI TERZE PARTI

I Servizi possono contenere link a siti web o servizi di terze parti. Questa Informativa sulla Privacy non si applica a queste terze parti.

Ti consigliamo di consultare le informative sulla privacy di eventuali servizi di terze parti che utilizzi.

---

## 12. DIRITTI PRIVACY CALIFORNIA (CCPA)

Se sei residente in California, hai diritti aggiuntivi ai sensi del California Consumer Privacy Act (CCPA):

### 12.1 Diritto di Sapere
Puoi richiedere informazioni su:
- Categorie di informazioni personali raccolte
- Scopi della raccolta delle informazioni
- Categorie di terze parti con cui condividiamo le informazioni

### 12.2 Diritto di Cancellazione
Puoi richiedere la cancellazione delle tue informazioni personali.

### 12.3 Diritto di Opt-Out
Puoi opporti alla "vendita" di informazioni personali. **Tuttavia, NON vendiamo le tue informazioni personali.**

### 12.4 Diritto alla Non Discriminazione
Non ti discrimineremo per l'esercizio dei tuoi diritti CCPA.

### 12.5 Categorie di Informazioni Personali Raccolte

| Categoria | Raccolto | Venduto | Scopo Commerciale |
|-----------|----------|---------|-------------------|
| Identificatori (email) | Sì | No | Gestione account |
| Attività Internet (IP, token) | Sì | No | Fornitura servizio, sicurezza |
| Geolocalizzazione (solo basata su IP) | Sì | No | Sicurezza |
| Inferenze | No | No | N/A |

**Per esercitare i tuoi diritti CCPA:**  
Email: niccolo.pagano@studbocconi.it  
Oggetto: "Richiesta CCPA"

Verificheremo la tua identità prima di elaborare la tua richiesta.

---

## 13. DIRITTI PRIVACY EUROPEI (GDPR)

Se sei residente nello Spazio Economico Europeo (SEE), Regno Unito o Svizzera, hai diritti ai sensi del Regolamento Generale sulla Protezione dei Dati (GDPR):

### 13.1 Basi Giuridiche per il Trattamento

Trattiamo i tuoi dati sulla base di:

| Base Giuridica | Attività di Trattamento |
|----------------|------------------------|
| **Contratto** | Creazione account, fornitura servizio |
| **Legittimo Interesse** | Sicurezza, prevenzione frodi, miglioramento servizio |
| **Obbligo Legale** | Conformità alle leggi |
| **Consenso** | Dove specificamente richiesto |

### 13.2 I Tuoi Diritti GDPR

- **Accesso** (Art. 15) - Richiedere una copia dei tuoi dati
- **Rettifica** (Art. 16) - Correggere dati inaccurati
- **Cancellazione** (Art. 17) - Richiedere la cancellazione ("diritto all'oblio")
- **Limitazione** (Art. 18) - Limitare il trattamento
- **Portabilità** (Art. 20) - Ricevere i dati in formato portatile
- **Opposizione** (Art. 21) - Opporsi al trattamento basato su legittimo interesse
- **Revoca Consenso** (Art. 7) - Revocare il consenso in qualsiasi momento

### 13.3 Titolare del Trattamento

**Titolare del Trattamento:**  
Niccolò Pagano  
Email: niccolo.pagano@studbocconi.it

### 13.4 Autorità di Controllo

Hai il diritto di presentare reclamo alla tua autorità locale per la protezione dei dati.

Per l'Italia: **Garante per la protezione dei dati personali**  
Sito web: https://www.garanteprivacy.it

### 13.5 Valutazione d'Impatto sulla Protezione dei Dati

Data la natura limitata dei dati che raccogliamo e l'assenza di attività di trattamento ad alto rischio, non è richiesta una DPIA formale. Tuttavia, valutiamo regolarmente le nostre pratiche sui dati.

---

## 14. MODIFICHE A QUESTA INFORMATIVA

Possiamo aggiornare questa Informativa sulla Privacy di tanto in tanto. Quando lo facciamo:

- Aggiorneremo la data "Ultimo aggiornamento" in alto
- Per modifiche sostanziali, ti notificheremo tramite:
  - Email al tuo indirizzo registrato
  - Notifica nell'App

L'uso continuato dei Servizi dopo che le modifiche diventano effettive costituisce accettazione dell'Informativa aggiornata.

---

## 15. CONTATTACI

Se hai domande, dubbi o richieste riguardo a questa Informativa sulla Privacy o ai tuoi dati personali, contattaci:

**Niccolò Pagano (NickP005)**  
Email: niccolo.pagano@studbocconi.it  
Sito web: https://plynx.cc

**Tempo di Risposta:**  
Ci impegniamo a rispondere a tutte le richieste relative alla privacy entro 30 giorni.

---

## APPENDICE: RIEPILOGO TRATTAMENTO DATI

### Dati che Raccogliamo

| Dato | Fonte | Scopo | Base Giuridica | Conservazione |
|------|-------|-------|----------------|---------------|
| Email | Fornita da te | ID account, login | Contratto | Fino alla cancellazione |
| Hash password | Fornita da te | Autenticazione | Contratto | Fino alla cancellazione |
| Indirizzo IP | Automatico | Sicurezza | Legittimo interesse | 90 giorni |
| Token dispositivi | Automatico | Auth hardware | Contratto | Fino alla cancellazione |
| Dati widget | I tuoi dispositivi | Funzione servizio | Contratto | Fino alla cancellazione |
| Dati storici | I tuoi dispositivi | Grafici | Contratto | Fino alla cancellazione |

### Dati che NON Raccogliamo

- Nome, telefono, indirizzo
- Dati di pagamento
- Posizione (GPS)
- Contatti, foto
- Dati analytics/tracking
- Identificatori pubblicitari

---

**UTILIZZANDO PLYNX, RICONOSCI DI AVER LETTO E COMPRESO QUESTA INFORMATIVA SULLA PRIVACY.**

---

## REGISTRO MODIFICHE

| Data | Modifiche |
|------|-----------|
| 26 novembre 2025 | Versione iniziale |

---

Copyright 2025 Plynx / Niccolò Pagano.
